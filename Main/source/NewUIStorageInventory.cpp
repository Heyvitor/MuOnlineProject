...
bool CNewUIStorageInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if (false == m_pNewInventoryCtrl->Create(
		g_pNewUI3DRenderMng, g_pNewItemMng, this, x + 15, y + 40, 8, 15))
	{
	...
	for (int i = BTN_INSERT_ZEN; i < BTN_LEFT; ++i)
	{
	...
	m_abtn[BTN_LEFT].ChangeButtonImgState(true, IMAGE_STORAGE_BTN_LEFT);
	m_abtn[BTN_LEFT].ChangeButtonInfo(x + 20, y + 10, 36, 29);
	m_abtn[BTN_LEFT].ChangeToolTipText(GlobalText[3163], true);

	m_abtn[BTN_RIGHT].ChangeButtonImgState(true, IMAGE_STORAGE_BTN_RIGHT);
	m_abtn[BTN_RIGHT].ChangeButtonInfo(x + 145, y + 10, 36, 29);
	m_abtn[BTN_RIGHT].ChangeToolTipText(GlobalText[3164], true);

	m_abtn[BTN_CHANGE].ChangeButtonImgState(true, IMAGE_STORAGE_BTN_CHANGE);
	m_abtn[BTN_CHANGE].ChangeButtonInfo(x + 23, y + 390, 36, 29);
	m_abtn[BTN_CHANGE].ChangeToolTipText(GlobalText[3165], true);

	m_abtn[BTN_BUY].ChangeButtonImgState(true, IMAGE_STORAGE_BTN_BUY);
	m_abtn[BTN_BUY].ChangeButtonInfo(x + 53, y + 390, 36, 29);
	m_abtn[BTN_BUY].ChangeToolTipText(GlobalText[3166], true);
...
void CNewUIStorageInventory::RenderBackImage()
{
	...
	RenderImage(IMAGE_STORAGE_MONEY, m_Pos.x + 60, m_Pos.y + 342, 170.f, 46.f);
	RenderImage(IMAGE_STORAGE_MONEY, m_Pos.x + 60, m_Pos.y + 364, 170.f, 46.f);
}
...
void CNewUIStorageInventory::RenderText()
{
	...
	g_pRenderText->SetTextColor(240, 198, 12, 255);
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 21, szTemp, STORAGE_WIDTH, 0, RT3_SORT_CENTER);
	...
	g_pRenderText->RenderText(m_Pos.x + 10 + 15, m_Pos.y + 342 + 8, "Zen");
	
	g_pRenderText->SetTextColor(240, 64, 64, 255);
    g_pRenderText->RenderText(m_Pos.x+10+15, m_Pos.y+342+29, "Taxa"/*GlobalText[266]*/);
...
void CNewUIStorageInventory::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_STORAGE_BTN_LEFT);
	DeleteBitmap(IMAGE_STORAGE_BTN_RIGHT);
	DeleteBitmap(IMAGE_STORAGE_BTN_CHANGE);
	DeleteBitmap(IMAGE_STORAGE_BTN_BUY);
}
...
bool SEASON3B::CNewUIStorageInventory::ProcessBtns()
{
	...
	else if (m_abtn[BTN_CHANGE].UpdateMouseEvent())
	{
		SEASON3B::CreateMessageBox(
			MSGBOX_LAYOUT_CLASS(SEASON3B::CChangeVaultMsgBoxLayout));
		return true;
	}
	else if (m_abtn[BTN_BUY].UpdateMouseEvent())
	{
