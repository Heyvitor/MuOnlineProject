		...
		case 0x05:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				...
				case 0x30:
					...
					break;
				case 0x40:
					gTradeX.GDTradeXItemSaveRecv((SDHP_TRADEX_ITEM_SAVE_RECV*)lpMsg);
					break;
				case 0x41:
					gTradeX.GCTradeXGetItemSave((SDHP_TRADEX_GET_ITEM_SAVE_RECV*)lpMsg, index);
					break;
				case 0x60:
					gChaosBoxGenesis.GCChaosBoxGenesisGetItemSaveRecv((SDHP_CHAOS_BOX_GENESIS_ITEM_SAVE_RECV*)lpMsg, index);
					break;
				case 0x61:
					gChaosBoxGenesis.GCChaosBoxGenesisItemSaveRecv((SDHP_CHAOS_BOX_GENESS_ITEM_SAVE_RECV*)lpMsg);
					break;
			}
			break;
		case 0x07:
		...
		case 0x18:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				...
				case 0x34:
					...
					break;
				case 0x40:
					gDeathSystem.GDInventoryGetItemCount((DEATH_SYSTEM_INVENTORY_RECV*)lpMsg, index);
					break;
				case 0x41:
					gDeathSystem.GDWarehouseGetItemCount((DEATH_SYSTEM_INVENTORY_RECV*)lpMsg, index);
					break;
				case 0x42:
					gDeathSystem.UpdateKillsSword((DEATH_SYSTEM_UPDATE_KILL_RECV*)lpMsg, index);
					break;
				case 0x43:
					gDeathSystem.GDChaosBoxGenesisGetItemCount((DEATH_SYSTEM_INVENTORY_RECV*)lpMsg, index);
					break;
				case 0x44:
					gDeathSystem.GDTradeXGetItemCount((DEATH_SYSTEM_INVENTORY_RECV*)lpMsg, index);
					break;
			}
			break;
		case 0x19:
...
void GDCharacterListRecv(SDHP_CHARACTER_LIST_RECV* lpMsg,int index) // OK
{
	...
	pMsg.MaxCharacter = (BYTE)gQueryManager.GetAsInteger("MaxCharacter");
	...
	// Banco agora suporta apenas GameID1..GameID5
	const int MaxDbCharacterSlots = 5;
	const int SlotsToUse = ((MaxCharacters > MaxDbCharacterSlots) ? MaxDbCharacterSlots : MaxCharacters);

	for (int n = 0; n < SlotsToUse; n++)
	{
...
void GDCharacterCreateRecv(SDHP_CHARACTER_CREATE_RECV* lpMsg, int index) // OK
{
	...
	else
	{
		...
		{
			// Banco agora suporta apenas GameID1..GameID5
			if (pMsg.slot >= MaxDbCharacterSlots)
			{
				pMsg.result = 2;
			}
			else
			if (pMsg.slot >= MaxCharacters)
			{
				pMsg.result = 2;
			}
			else
			{
				if (gQueryManager.ExecQuery("SELECT MaxCharacter FROM AccountCharacter WHERE Id='%s'", lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
				{
					gQueryManager.Close();
				}
				else
				{
					int MaxCharacterAccount = gQueryManager.GetAsInteger("MaxCharacter");

					gQueryManager.Close();

					if (pMsg.slot >= MaxCharacterAccount)
					{
						pMsg.result = 2;
	...
	else
	{
		...
		if (pMsg.result == 1)
		{
			...
			else
			{
				...
				{
					if (slot >= MaxDbCharacterSlots)
					{
						// slot fora do range suportado pelo banco
						return;
					}
